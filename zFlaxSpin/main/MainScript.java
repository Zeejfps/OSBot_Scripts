...
public class MainScript extends Script{
	...
	private void changeFloor(int correctFloor){
	...
	private void walkToDestination(final Area area, final Position destination) throws InterruptedException{
		...
		if(client.getMyPlayer().isInArea(area)){
			//log("In area.");
			return;
		}
		walk(destination);
		...
		if(client.getMyPlayer().getZ() == 1){
			...
			if(door != null){
				...
				door.interact("Open");
				sleep(new TillDoorOpen(door), 5000);
			}
		}
	...
	private void performTask(final Task task){
		//log("Performing a task...");
		if(task.equals(Task.BANK)){
			final NPC banker = closestNPC(4907);
			try{
			...
			try {
				...
				type(Integer.toString(amountHad));
	...
	private void clickOnSpinningWheel(final RS2Object spinningWheel) throws InterruptedException{
		if(client.getInterface(interfaceParent).isValid()){
			return;
		}	
		selectOption(spinningWheel, spinningWheel.getMouseDestination(), "Spin");
		sleep(new TillValid(this, interfaceParent), 3000);
		clickOnSpinningWheel(spinningWheel);
	}
	
	private void openBank(final NPC banker) throws InterruptedException{
		if(client.getBank().isOpen()){
			return;
		}
		banker.interact(false, "Bank");
		sleep(new TillBankOpen(this), 2000);
		counter++;
		if(counter > 2){
			return;
		}
